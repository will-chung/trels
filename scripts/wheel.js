...
class Wheel {
    ...
    combine() {
        ...
        if (this.level != 0) {
            ...
            prevWheel.sectors.forEach(sector => {
                ...
                let i = 0;
                while (i < this.sectors.length && this.sectors[i].startAngle < startAngle)
                    i++;
    ...
    combineRegion(region, startIndex, deleteCount) {
        ...
        for (let j = 0; j < region.length; j++) {
            ...
            if (j > 0)
                sector.startAngle = region[j-1].endAngle;
            ...
            region.splice(j, 1, new Sector(sector.value, sector.startAngle, sector.startAngle + arcAngle, sector.color, sector.wheel));
        }
    ...
    copy() {
        ...
        this.sectors.forEach(sector => {
            copyWheel.addSector(new Sector(sector.value, sector.startAngle, sector.endAngle, sector.defaultColor, copyWheel));
        });
